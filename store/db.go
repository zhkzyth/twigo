...
func NewDB() (*DB, error) {
	...
	d, err := sqlx.Open("mysql", fmt.Sprintf("%s:%s@tcp(%s:%s)/%s?parseTime=true&multiStatements=true", DB_USER, DB_PASSWD, DB_HOST, DB_PORT, DB_NAME))
	if err != nil {
		return nil, err
	}
	db := &DB{db: d}
